...
namespace Wiener
{
    class ContinuedFractionAttack
    {
        ...
        public BigInteger WienerAlgorithm()
        {
            ...
            for (int i = 0; i < iCount; i++)
            {
                BigFraction tmp_fraction = new BigFraction();
                BigFraction _bfrK_DG = new BigFraction();
                BigFraction _bfrEulerToitient; // phi = edg / k
                ...
                _bfrEulerToitient = new BigFraction(_bfrK_DG.Denominator * m_rsaKey.Public_exponent, _bfrK_DG.Numerator);

                // Some paper use the nearest value, another paper use the smallest value
                //BigInteger _iEulerToitient = _bfrEulerToitient.Round();
                ...
                BigInteger _bintAnpha = BigInteger.Divide((m_rsaKey.Modulus - _iEulerToitient + 1), 2);
                BigInteger _biBeta = BigInteger.Pow(_bintAnpha, 2) - m_rsaKey.Modulus;
                ...
                m_rsaKey.Private_exponent = BigInteger.Divide(_bfrK_DG.Denominator, _iG);
